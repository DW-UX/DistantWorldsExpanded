...
namespace System.Runtime.Serialization
{
    internal static class XmlConverter
    {
        ...
        private static Base64Encoding _base64Encoding;

        public static Base64Encoding Base64Encoding
        {
            get
            {
                if (_base64Encoding == null)
                {
                    _base64Encoding = new Base64Encoding();
                }
        ...
        private static UnicodeEncoding UnicodeEncoding
        {
            get
            {
                if (_unicodeEncoding == null)
                {
                    _unicodeEncoding = new UnicodeEncoding(false, false, true);
                }
        ...
        {
            if (TryParseInt32(buffer, offset, count, out int value))
            {
                return value;
            }
        ...
        {
            if (TryParseInt64(buffer, offset, count, out long value))
            {
                return value;
            }
        ...
        {
            if (TryParseSingle(buffer, offset, count, out float value))
            {
                return value;
            }
        ...
        {
            if (TryParseDouble(buffer, offset, count, out double value))
            {
                return value;
            }
        ...
        {
            if (TryParseDateTime(buffer, offset, count, out DateTime value))
            {
                return value;
            }
        ...
        private static bool TryParseInt64(byte[] chars, int offset, int count, out long result)
        {
            ...
            if (count < 11)
            {
                if (!TryParseInt32(chars, offset, count, out int value))
                {
                    return false;
                }
