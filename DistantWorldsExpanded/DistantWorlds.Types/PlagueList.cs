        ...
        public void LoadFromFile(SQLiteDataReader reader)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    string name = reader.GetString(reader.GetOrdinal("PlagueName"));
                    int pictureRef = reader.GetInt32(reader.GetOrdinal("PictureRef"));
                    float mortalityRate = reader.GetFloat(reader.GetOrdinal("MortalityRate"));
                    int infectionChance = reader.GetInt32(reader.GetOrdinal("InfectionChance"));
                    int duration = reader.GetInt32(reader.GetOrdinal("Duration"));
                    int naturalOcurence = reader.GetInt32(reader.GetOrdinal("NaturalOccurrenceLevel"));
                    bool canCompletelyEliminatePopulation = reader.GetBoolean(reader.GetOrdinal("CanCompletelyEliminatePopulation"));
                    string exceptionRaceName = reader.GetString(reader.GetOrdinal("ExceptionRaceName"));
                    float exceptionMortalityRate = reader.GetFloat(reader.GetOrdinal("ExceptionMortalityRate"));
                    int exceptionInfectionChance = reader.GetInt32(reader.GetOrdinal("ExceptionInfectionChance"));
                    int exceptionDuration = reader.GetInt32(reader.GetOrdinal("ExceptionDuration"));
                    int specialFunctionCode = reader.GetInt32(reader.GetOrdinal("SpecialFunctionCode"));
                    string description = reader.GetString(reader.GetOrdinal("Description"));
                    if (id < 0)
                        throw new ApplicationException($"Could not read PlagueId at ID {id}");
                    if (string.IsNullOrWhiteSpace(name))
                        throw new ApplicationException($"Could not read Name at ID {id}");
                    if (pictureRef < 0)
                        throw new ApplicationException($"Could not read PictureRef at ID {id}");
                    if (mortalityRate < 0)
                        throw new ApplicationException($"Could not read MortalityRate at ID {id}");
                    if (mortalityRate < 0.001 || mortalityRate > 5.0)
                        throw new ApplicationException($"Invalid MortalityRate. Should be in range 0.001 to 5.0. At ID {id}");
                    if (infectionChance < 0)
                        throw new ApplicationException($"Could not read InfectionChance at ID {id}");
                    if (duration < 0)
                        throw new ApplicationException($"Could not read Duration at ID {id}");
                    if (naturalOcurence < 0)
                        throw new ApplicationException($"Could not read naturalOcurence at ID {id}");
                    //if (string.IsNullOrWhiteSpace(exceptionRaceName))
                    //    throw new ApplicationException($"Could not read ExceptionRaceName at ID {id}");
                    if (exceptionMortalityRate < 0)
                        throw new ApplicationException($"Could not read ExceptionMortalityRate at ID {id}");
                    if (!string.IsNullOrWhiteSpace(exceptionRaceName) && (exceptionMortalityRate < 0.001 || exceptionMortalityRate > 5.0))
                        throw new ApplicationException($"Invalid ExceptionMortalityRate. Should be in range 0.001 to 5.0. At ID {id}");
                    if (exceptionInfectionChance < 0)
                        throw new ApplicationException($"Could not read ExceptionInfectionChance at ID {id}");
                    if (exceptionDuration < 0)
                        throw new ApplicationException($"Could not read ExceptionDuration at ID {id}");
                    if (specialFunctionCode < 0)
                        throw new ApplicationException($"Could not read SpecialFunctionCode at ID {id}");
                    if (string.IsNullOrWhiteSpace(description))
                        throw new ApplicationException($"Could not read SpecialFunctionCode at ID {id}");

                    plagueList.Add(new Plague((byte)id, name, pictureRef, mortalityRate, infectionChance, duration)
                    {
                        CanCompletelyEliminatePopulation = canCompletelyEliminatePopulation,
                        NaturalOccurrenceLevel = naturalOcurence,
                        ExceptionRaceName = exceptionRaceName,
                        ExceptionMortalityRate = exceptionMortalityRate,
                        ExceptionInfectionChance = exceptionInfectionChance,
            ...
            plagueList.Sort();
            if (!this.CheckSequentialIds(plagueList))
                throw new ApplicationException("Non-sequential Plague IDs detected. Plague ID values must start at 0 (zero) and be sequential.");
            this.AddRange((IEnumerable<Plague>)plagueList);
        }
