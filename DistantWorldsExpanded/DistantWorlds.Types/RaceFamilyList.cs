...
namespace DistantWorlds.Types
{
    ...
    public class RaceFamilyList : List<RaceFamily>
    {
        ...
        public List<int> GetIdsBySpecialFunctionCode(int specialFunctionCode)
        {
            List<int> specialFunctionCode1 = new List<int>();
        ...
        public void LoadFromFile(SQLiteDataReader reader)
        {
            ...
            try
            {
                ...
                {
                    string name = "";
                    int specialFuncCode = 0;
                    raceId = reader.GetInt32(0);
                    if (raceId < 0)
                        throw new ApplicationException($"Wrong RaceFamilyId at ID {raceId}");
                    name = reader.GetString(1);
                    if (string.IsNullOrEmpty(name))
                        throw new ApplicationException($"Could not read Name at ID {raceId}, Value - {name}");
                    specialFuncCode = reader.GetInt32(2);

                    RaceFamily raceFamily = new RaceFamily(raceId, name, specialFuncCode);
                    raceFamilyList.Add(raceFamily);
            ...
            {
                throw new ApplicationException($"Error at ID {raceId}");
            }
            raceFamilyList.Sort();
            if (!this.CheckSequentialIds(raceFamilyList))
                throw new ApplicationException("Non-sequential Race Family IDs detected. Race Family ID values must start at 0 (zero) and be sequential.");
            this.AddRange((IEnumerable<RaceFamily>)raceFamilyList);
            this.SetDefaultBiases();
        }
