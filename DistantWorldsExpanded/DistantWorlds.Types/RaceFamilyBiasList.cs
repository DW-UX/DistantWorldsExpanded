...
namespace DistantWorlds.Types
{
    ...
    public class RaceFamilyBiasList
    {
        ...
        public static void LoadFromFile(SQLiteDataReader reader, ref RaceFamilyList raceFamilies)
        {
                ...
                while (reader.Read())
                {
                    ...
                    //if (!string.IsNullOrEmpty(str) && str.Trim() != string.Empty && str.Trim().Substring(0, 1) != "'")
                    {
                        ...
                        if (raceId >= 0 && intList.Max() < raceFamilies.Count)
                        {
                            List<KeyValuePair<int, int>> biases = new List<KeyValuePair<int, int>>();
                            for (int index = 0; index < intList.Count; ++index)
                                biases.Add(new KeyValuePair<int, int>(raceId, intList[index]));
                            raceFamilies[raceId].Biases.SetInternal(biases);
                        }
                    }
                }
                //streamReader.Close();
            ...
            {
                throw new ApplicationException($"Error at raceId {raceId}");
            }
        }
