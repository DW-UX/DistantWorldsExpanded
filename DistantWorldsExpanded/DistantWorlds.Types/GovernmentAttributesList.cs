        ...
        public void LoadFromFile(SQLiteDataReader reader)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (id < 0)
                        throw new ApplicationException($"Wrong GovernmentId {id}");
                    string name = reader.GetString(reader.GetOrdinal("Name"));
                    if (string.IsNullOrEmpty(name))
                        throw new ApplicationException($"Could not read Name at ID {id}");
                    double corruptionLvl = reader.GetDouble(reader.GetOrdinal("Corruption"));
                    if (corruptionLvl < 0)
                        throw new ApplicationException($"Could not read Corruption at ID {id}");
                    if (corruptionLvl < 0.0 || corruptionLvl > 3.0)
                        throw new ApplicationException($"Invalid Corruption value (must be between 0 and 3.0) at ID {id}");
                    double warWeariness = reader.GetDouble(reader.GetOrdinal("WarWeariness"));
                    if (warWeariness < 0)
                        throw new ApplicationException($"Could not read War Weariness at ID {id}");
                    if (warWeariness < 0.0 || warWeariness > 3.0)
                        throw new ApplicationException($"Invalid War Weariness value (must be between 0 and 3.0) at ID {id}");
                    double maintenanceCosts = reader.GetDouble(reader.GetOrdinal("MaintenanceCosts"));
                    if (maintenanceCosts < 0)
                        throw new ApplicationException($"Could not read Maintenance Costs at ID {id}");
                    if (maintenanceCosts < 0.0 || maintenanceCosts > 3.0)
                        throw new ApplicationException($"Invalid Maintenance Costs value (must be between 0 and 3.0) at ID {id}");
                    double approvalRating = reader.GetDouble(reader.GetOrdinal("ApprovalRating"));
                    if (approvalRating < 0)
                        throw new ApplicationException($"Could not read Approval Rating at ID {id}");
                    if (approvalRating < 0.0 || approvalRating > 3.0)
                        throw new ApplicationException($"Invalid Approval Rating value (must be between 0 and 3.0) at ID {id}");
                    double populationGrowth = reader.GetDouble(reader.GetOrdinal("PopulationGrowth"));
                    if (populationGrowth < 0)
                        throw new ApplicationException($"Could not read Population Growth at ID {id}");
                    if (populationGrowth < 0.0 || populationGrowth > 3.0)
                        throw new ApplicationException($"Invalid Population Growth value (must be between 0 and 3.0) at ID {id}");
                    double researchSpeed = reader.GetDouble(reader.GetOrdinal("ResearchSpeed"));
                    if (researchSpeed < 0)
                    ...
                    if (troopRecruitment < 0.0 || troopRecruitment > 3.0)
                        throw new ApplicationException($"Invalid Troop Recruitment value (must be between 0 and 3.0) at ID {id}");
                    double tradeBonus = reader.GetDouble(reader.GetOrdinal("TradeBonus"));
                    if (tradeBonus < 0)
                        throw new ApplicationException($"Could not read TradeBonus at ID {id}");
                    if (tradeBonus < 0.0 || tradeBonus > 3.0)
                        throw new ApplicationException($"Invalid Trade Bonus value (must be between 0 and 3.0) at ID {id}");
                    double leaderReplacementLikeliness = reader.GetDouble(reader.GetOrdinal("LeaderReplacementLikeliness"));
                    if (leaderReplacementLikeliness < 0)
                        throw new ApplicationException($"Could not read TradeBonus at ID {id}");
                    if (leaderReplacementLikeliness < 0.0 || leaderReplacementLikeliness > 3.0)
                        throw new ApplicationException($"Invalid Leader Replacement Likeliness value (must be between 0 and 3.0) at ID {id}");
                    double leaderReplacementDisruptionLevel = reader.GetDouble(reader.GetOrdinal("LeaderReplacementDisruptionLevel"));
                    if (leaderReplacementDisruptionLevel < 0)
                        throw new ApplicationException($"Could not read Leader Replacement Disruption at ID {id}");
                    if (leaderReplacementDisruptionLevel < 0.0 || leaderReplacementDisruptionLevel > 3.0)
                        throw new ApplicationException("Invalid Leader Replacement Disruption Level value (must be between 0 and 3.0) at ID {id}");
                    double leaderReplacementBoost = reader.GetDouble(reader.GetOrdinal("LeaderReplacementBoost"));
                    if (leaderReplacementBoost < 0)
                        throw new ApplicationException($"Could not read Leader Replacement Boost at ID {id}");
                    if (leaderReplacementBoost < 0.0 || leaderReplacementBoost > 3.0)
                        throw new ApplicationException($"Invalid Leader Replacement Boost Level value (must be between 0 and 3.0) at ID {id}");
                    int leaderReplacementCharacterPool = reader.GetInt32(reader.GetOrdinal("LeaderReplacementCharacterPool"));
                    if (leaderReplacementCharacterPool < 0)
                        throw new ApplicationException($"Could not read Leader Replacement Character Pool at ID {id}");
                    if (leaderReplacementCharacterPool < 0 || leaderReplacementCharacterPool > 3)
                        throw new ApplicationException($"Invalid Leader Replacement Character Pool value (must be between 0 and 3) at ID {id}");
                    int leaderReplacementTypicalManner = reader.GetInt32(reader.GetOrdinal("LeaderReplacementTypicalManner"));
                    if (leaderReplacementTypicalManner < 0)
                        throw new ApplicationException($"Could not read Leader Replacement Typical Manner at ID {id}");
                    if (leaderReplacementTypicalManner < 0 || leaderReplacementTypicalManner > 2)
                        throw new ApplicationException($"Invalid Leader Replacement Typical Manner value (must be between 0 and 2) at ID {id}");
                    double stability = reader.GetDouble(reader.GetOrdinal("Stability"));
                    if (stability < 0)
                        throw new ApplicationException($"Could not Stability at ID {id}");
                    if (stability < 0.0 || stability > 3.0)
                        throw new ApplicationException($"Invalid Stability value (must be between 0 and 3.0) at ID {id}");
                    double concernForOwnReputation = reader.GetDouble(reader.GetOrdinal("PopulationsConcernOverOwnReputation"));
                    if (concernForOwnReputation < 0)
                        throw new ApplicationException($"Could not ConcernForOwnReputation at ID {id}");
                    if (concernForOwnReputation < 0.0 || concernForOwnReputation > 2.0)
                        throw new ApplicationException($"Invalid Concern For Own Reputation value (must be between 0 and 2.0) at line ID {id}");
                    double importanceOfOthersReputations = reader.GetDouble(reader.GetOrdinal("ImportanceOfOtherEmpiresReputations"));
                    if (importanceOfOthersReputations < 0)
                        throw new ApplicationException($"Could not Importance Of Others Reputations at ID {id}");
                    if (importanceOfOthersReputations < 0.0 || importanceOfOthersReputations > 2.0)
                        throw new ApplicationException("Invalid Importance Of Others Reputations value (must be between 0 and 2.0) at line ID {id}");
                    int specialFunctionCode = reader.GetInt32(reader.GetOrdinal("SpecialFunctionCode"));
                    if (specialFunctionCode < 0)
                        throw new ApplicationException($"Could not SpecialFunctionCodeat ID {id}");
                    if (specialFunctionCode < 0 || specialFunctionCode > 1)
                        throw new ApplicationException($"Invalid Special Function Code value (must be between 0 and 1) at ID {id}");
                    int availability = reader.GetInt32(reader.GetOrdinal("Availability"));
                    if (availability < 0)
                        throw new ApplicationException($"Could not Availability ID {id}");
                    if (availability < 0 || availability > 3)
                        throw new ApplicationException("Invalid Availability value (must be between 0 and 3) at ID {id}");
                    empireNameAdjectives.AddRange(reader.GetString(reader.GetOrdinal("EmpireNameAdjectives")).Split(',', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries));
                    empireNameNouns.AddRange(reader.GetString(reader.GetOrdinal("EmpireNameNouns")).Split(',', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries));

                    GovernmentAttributes governmentAttributes = new GovernmentAttributes(id, name, corruptionLvl, warWeariness, maintenanceCosts, approvalRating, populationGrowth, researchSpeed, troopRecruitment, tradeBonus, leaderReplacementLikeliness, leaderReplacementDisruptionLevel, leaderReplacementBoost, leaderReplacementCharacterPool, leaderReplacementTypicalManner, stability, concernForOwnReputation, importanceOfOthersReputations, specialFunctionCode, availability, empireNameAdjectives, empireNameNouns);
                    governmentAttributesList.Add(governmentAttributes);
            ...
            {
                throw new ApplicationException($"Error at ID {id}");
            }
            governmentAttributesList.Sort();
            if (!this.CheckSequentialIds(governmentAttributesList))
                throw new ApplicationException("Non-sequential Government IDs detected. Government ID values must start at 0 (zero) and be sequential.");
            this.AddRange((IEnumerable<GovernmentAttributes>)governmentAttributesList);
            this.SetDefaultBiases();
        }
