        ...
        public void LoadFromFile(SQLiteDataReader reader)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (id < 0)
                        throw new ApplicationException($"Wrong FighterId {id}");
                    ...
                    int fighterTypeVal = reader.GetInt32(reader.GetOrdinal("Type"));
                    if (fighterTypeVal < 0)
                        throw new ApplicationException($"Could not read Type at {id}");
                    FighterType fighterType = fighterTypeVal switch
                    {
                        0 => FighterType.Interceptor,
                        1 => FighterType.Bomber,
                        _ => throw new ApplicationException($"Invalid Type at ID {id} of file Fighters. Type should be either 0 (Interceptor) or 1 (Bomber).")
                    };

                    double techLevel = reader.GetDouble(reader.GetOrdinal("TechLevel"));
                    if (techLevel < 0)
                        throw new ApplicationException($"Could not read TechLevel at {id}");

                    short energyCapacity = (short)reader.GetInt32(reader.GetOrdinal("EnergyCapacity"));
                    if (energyCapacity < 0)
                        throw new ApplicationException($"Could not read EnergyCapacity at {id}");

                    float energyRechargeRate = reader.GetFloat(reader.GetOrdinal("EnergyRechargeRate"));
                    if (energyRechargeRate < 0)
                        throw new ApplicationException($"Could not read EnergyRechargeRate at {id}");

                    short topSpeed = (short)reader.GetInt32(reader.GetOrdinal("TopSpeed"));
                    if (topSpeed < 0)
                        throw new ApplicationException($"Could not read TopSpeed at {id}");

                    float topSpeedEnergyConsumptionRate = reader.GetFloat(reader.GetOrdinal("TopSpeedEnergyConsumptionRate"));
                    ...
                    float turnRate = reader.GetFloat(reader.GetOrdinal("TurnRate"));
                    if (turnRate < 0)
                        throw new ApplicationException($"Could not read TurnRate at {id}");

                    short engineExhaustImageIndex = (short)reader.GetInt32(reader.GetOrdinal("EngineExhaustImageIndex"));
                    if (engineExhaustImageIndex < 0)
                        throw new ApplicationException($"Could not read EngineExhaustImageIndex at {id}");

                    short shieldsCapacity = (short)reader.GetInt32(reader.GetOrdinal("ShieldsCapacity"));
                    if (shieldsCapacity < 0)
                        throw new ApplicationException($"Could not read ShieldsCapacity at {id}");

                    float shieldRechargeRate = reader.GetFloat(reader.GetOrdinal("ShieldRechargeRate"));
                    if (shieldRechargeRate < 0)
                        throw new ApplicationException($"Could not read ShieldRechargeRate at {id}");

                    short damageRepairRate = (short)reader.GetInt32(reader.GetOrdinal("DamageRepairRate"));
                    if (damageRepairRate < 0)
                        throw new ApplicationException($"Could not read DamageRepairRate at {id}");

                    short countermeasureModifier = (short)reader.GetInt32(reader.GetOrdinal("CountermeasureModifier"));
                    if (countermeasureModifier < 0)
                        throw new ApplicationException($"Could not read CountermeasureModifier at {id}");

                    short targettingModifier = (short)reader.GetInt32(reader.GetOrdinal("TargettingModifier"));
                    if (targettingModifier < 0)
                        throw new ApplicationException($"Could not read TargettingModifier at {id}");

                    int WeaponTypeVal = reader.GetInt32(reader.GetOrdinal("WeaponType"));
                    if (countermeasureModifier < 0)
                        throw new ApplicationException($"Could not read WeaponType at {id}");
                    ComponentType componentType = WeaponTypeVal switch
                    {
                        0 => ComponentType.WeaponBeam,
                        1 => ComponentType.WeaponTorpedo,
                        2 => ComponentType.WeaponMissile,
                        _ => throw new ApplicationException($"Invalid WeaponType at ID {id} at Fighters. Should be 0 (beam), 1 (torpedo) or 2 (missile).")
                    };

                    int weaponImageIndex = reader.GetInt32(reader.GetOrdinal("WeaponImageIndex"));
                    if (weaponImageIndex < 0)
                        throw new ApplicationException($"Could not read WeaponImageIndex at {id}");

                    short weaponDamage = (short)reader.GetInt32(reader.GetOrdinal("WeaponDamage"));
                    if (weaponDamage < 0)
                        throw new ApplicationException($"Could not read WeaponDamage at {id}");

                    short weaponRange = (short)reader.GetInt32(reader.GetOrdinal("WeaponRange"));
                    if (weaponRange < 0)
                        throw new ApplicationException($"Could not read WeaponRange at {id}");

                    short weaponEnergyRequired = (short)reader.GetInt32(reader.GetOrdinal("WeaponEnergyRequired"));
                    if (weaponEnergyRequired < 0)
                        throw new ApplicationException($"Could not read WeaponEnergyRequired at {id}");

                    short weaponSpeed = (short)reader.GetInt32(reader.GetOrdinal("WeaponSpeed"));
                    if (weaponSpeed < 0)
                        throw new ApplicationException($"Could not read WeaponSpeed at {id}");

                    short weaponDamageLoss = (short)reader.GetInt32(reader.GetOrdinal("WeaponDamageLoss"));
                    if (weaponDamageLoss < 0)
                        throw new ApplicationException($"Could not read WeaponDamageLoss at {id}");

                    short weaponFireRate = (short)reader.GetInt32(reader.GetOrdinal("WeaponFireRate"));
                    if (weaponFireRate < 0)
                        throw new ApplicationException($"Could not read WeaponFireRate at {id}");

                    string weaponSoundFile = reader.GetString(reader.GetOrdinal("WeaponSoundEffectFilename"));


                    specificationList.Add(new FighterSpecification()
                    {
                        FighterSpecificationId = id,
                        Name = name,
                        Type = fighterType,
                        Size = 10,
                        TechLevel = techLevel,
                        ...
                        TopSpeed = topSpeed,
                        TopSpeedEnergyConsumptionRate = topSpeedEnergyConsumptionRate,
                        AccelerationRate = accelerationRate,
                        TurnRate = turnRate,
                        EngineExhaustImageIndex = engineExhaustImageIndex,
                        ShieldsCapacity = shieldsCapacity,
                        ShieldRechargeRate = shieldRechargeRate,
                        DamageRepairRate = damageRepairRate,
                        CountermeasureModifier = countermeasureModifier,
                        TargettingModifier = targettingModifier,
                        WeaponType = componentType,
                        WeaponImageIndex = weaponImageIndex,
                        WeaponDamage = weaponDamage,
                        WeaponRange = weaponRange,
                        WeaponEnergyRequired = weaponEnergyRequired,
                        WeaponSpeed = weaponSpeed,
                        WeaponDamageLoss = weaponDamageLoss,
                        WeaponFireRate = weaponFireRate,
                        WeaponSoundEffectFilename = weaponSoundFile
                    });
                }
            }
            catch (ApplicationException ex)
            ...
            {
                throw new ApplicationException($"Error at line ID {id} at Fighters");
            }
            specificationList.Sort();
            if (!this.CheckSequentialIds(specificationList))
                throw new ApplicationException("Non-sequential Fighter IDs detected. Fighter ID values must start at 0 (zero) and be sequential.");
            this.AddRange((IEnumerable<FighterSpecification>)specificationList);
        }
