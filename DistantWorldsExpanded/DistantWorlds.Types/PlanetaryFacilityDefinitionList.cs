        ...
        public void LoadFromFile(SQLiteDataReader reader)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (id < 0)
                        throw new ApplicationException($"Wrong FacilityId {id}");
                    ...
                    if (pictureRef < 0)
                        throw new ApplicationException($"Could not read PictureRef at ID {id} at Facility");
                    if (pictureRef < 0 || pictureRef > (int)short.MaxValue)
                        throw new ApplicationException($"Invalid PictureRef at ID {id} of Facility. PictureRef must be within range of 0 to 32767.");

                    double buildCost = reader.GetDouble(reader.GetOrdinal("BuildCost"));
                    if (buildCost < 0)
                        throw new ApplicationException($"Could not read BuildCost at ID {id} at Facility");

                    double maintenmanceCost = reader.GetDouble(reader.GetOrdinal("MaintenanceCost"));
                    if (maintenmanceCost < 0)
                        throw new ApplicationException($"Could not read MaintenanceCost at at ID {id} at Facility");

                    int value1 = reader.GetInt32(reader.GetOrdinal("Value1"));
                    if (value1 < 0)
                        throw new ApplicationException($"Could not read Value1 at ID {id} at Facility");

                    int value2 = reader.GetInt32(reader.GetOrdinal("Value2"));
                    if (value2 < 0)
                        throw new ApplicationException($"Could not read Value2 at ID {id} at Facility");

                    int value3 = reader.GetInt32(reader.GetOrdinal("Value3"));
                    if (value3 < 0)
                        throw new ApplicationException($"Could not read Value3 at ID {id} at Facility");

                    string description = reader.GetString(reader.GetOrdinal("Description"));
                    if (string.IsNullOrWhiteSpace(description))
                        throw new ApplicationException($"Could not read Description at ID {id} at Facility");



                    facilityDefinitionList.Add(new PlanetaryFacilityDefinition(id, name, type, (short)pictureRef, buildCost, value1, value2, value3)
                    {
                        WonderType = wonderType,
                        Maintenance = maintenmanceCost,
            ...
            {
                throw new ApplicationException($"Error at {id} at Facility");
            }
            facilityDefinitionList.Sort();
            if (!this.CheckSequentialIds(facilityDefinitionList))
                throw new ApplicationException("Non-sequential Planetary Facility IDs detected. Planetary Facility ID values must start at 0 (zero) and be sequential.");
            this.AddRange((IEnumerable<PlanetaryFacilityDefinition>)facilityDefinitionList);
        }
