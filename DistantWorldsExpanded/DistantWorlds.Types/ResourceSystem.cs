...
namespace DistantWorlds.Types
{
    ...
    public class ResourceSystem
    {
        ...
        public void CheckResourcesForConflictingTypesAtHabitats()
        {
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Ammonia, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Argon, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.BarrenRock, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.BarrenRock, true, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.CarbonDioxide, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Chlorine, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Continental, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Desert, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.FrozenGasGiant, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.GasGiant, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Helium, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Hydrogen, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Ice, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Ice, true, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.MarshySwamp, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Metal, true, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.NitrogenOxygen, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Ocean, false, false);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Oxygen, false, true);
            this.CheckHabitatTypeForMineralAndGasResources(HabitatType.Volcanic, false, false);
        }
          ...
          ResourceDefinitionList gasResources)
        {
            ...
            for (int index = 0; index < mineralResources.Count; ++index)
                str1 = str1 + mineralResources[index].Name + ", ";
            string str2 = str1.Substring(0, str1.Length - 2);
            return !habitatIsAsteroid
    ? (!habitatIsGasCloud
        ? $"Resources have incorrectly defined {Galaxy.ResolveDescription(habitatType)} Planets/Moons with both gas and mineral resources: {str2}"
        : $"Resources have incorrectly defined {Galaxy.ResolveDescription(habitatType)} Gas Clouds with both gas and mineral resources: {str2}")
    : $"Resources have incorrectly defined {Galaxy.ResolveDescription(habitatType)} Asteroids with both gas and mineral resources: {str2}";
          ...
          bool habitatIsGasCloud)
        {
            ...
            if (mineralResources.Count > 0 && gasResources.Count > 0)
                throw new ApplicationException(this.GenerateHabitatResourceConflictMessage(habitatType, habitatIsAsteroid, habitatIsGasCloud, mineralResources, gasResources));
        }
        ...
        public void LoadFromFile(SQLiteDataReader reader)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (id < 0)
                        throw new ApplicationException($"Wrong ResourceId {id}");

                    string name = reader.GetString(reader.GetOrdinal("Name"));
                    if (string.IsNullOrWhiteSpace(name))
                        throw new ApplicationException($"Could not read Name at {id} at Resourses");

                    int pictureRef = reader.GetInt32(reader.GetOrdinal("PictureRef"));
                    if (pictureRef < 0)
                        throw new ApplicationException($"Could not read PictureRef at {id} at Resourses");

                    float basePrice = reader.GetFloat(reader.GetOrdinal("BasePrice"));
                    if (basePrice < 0)
                        throw new ApplicationException($"Could not read BasePrice at {id} at Resourses");

                    ResourceGroup group;
                    int groupVal = reader.GetInt32(reader.GetOrdinal("TypeOfResource"));
                    if (groupVal < 0)
                        throw new ApplicationException($"Could not read Resource Group at {id} at Resourses");
                    switch (groupVal)
                    {
                        case 0:
                            group = ResourceGroup.Mineral;
                        ...
                        case 2:
                            ...
                            break;
                        default:
                            throw new ApplicationException($"Invalid Resource Group at {id} at Resourses");
                    }

                    int superLuxuryBonusAmount = reader.GetInt32(reader.GetOrdinal("SuperLuxuryBonusAmount"));
                    if (superLuxuryBonusAmount < 0)
                        throw new ApplicationException($"Could not read SuperLuxuryBonusAmount at {id} at Resourses");

                    bool isFuel = reader.GetBoolean(reader.GetOrdinal("IsFuel"));
                    bool isImportantPreWarpResource = reader.GetBoolean(reader.GetOrdinal("IsImportantPreWarpResource"));

                    float colonyGrowthResourceLevel = reader.GetFloat(reader.GetOrdinal("ColonyGrowthResourceLevel"));
                    if (colonyGrowthResourceLevel < 0)
                        throw new ApplicationException($"Could not read ColonyGrowthResourceLevel at {id} at Resourses");
                    if ((double)colonyGrowthResourceLevel < 0.0 || (double)colonyGrowthResourceLevel > 1.0)
                        throw new ApplicationException("Invalid ColonyGrowthResourceLevel at {id} at Resourses");

                    int colonyManufacturingLevel = reader.GetInt32(reader.GetOrdinal("ColonyManufacturingLevel"));
                    if (colonyManufacturingLevel < 0)
                        throw new ApplicationException($"Could not read ColonyManufacturingLevel at {id} at Resourses");
                    if (colonyManufacturingLevel < 0 || colonyManufacturingLevel > 10000)
                        throw new ApplicationException($"Invalid ColonyManufacturingLevel at {id} at Resourses");

                    ResourceDefinition resourceDefinition = new ResourceDefinition((byte)id, name, pictureRef, basePrice, group, superLuxuryBonusAmount, isFuel, isImportantPreWarpResource, colonyGrowthResourceLevel, colonyManufacturingLevel);
                    string[] distributionArr = reader.GetString(reader.GetOrdinal("Distribution")).Split(',', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);

                    if (distributionArr.Length % 5 != 0)
                    ...
                    for (int i = 0; i < distributionArr.Length; i += 5)
                    {
                        ...
                        ResourcePrevalence resourcePrevalence = new ResourcePrevalence(isAsteroid, isGasCloud, subType, prevalenceValue, abundanceMinimum, abundanceMaximum);
                        resourceDefinition.Prevalence.Add(resourcePrevalence);
                    }
            ...
            {
                throw new ApplicationException($"Error at ID {id} at Resourses");
            }
            resourceDefinitionList.Sort();
            if (!this.CheckSequentialIds(resourceDefinitionList))
                throw new ApplicationException("Non-sequential Resource IDs detected. Resource ID values must start at 0 (zero) and be sequential.");
            this.Resources.AddRange((IEnumerable<ResourceDefinition>)resourceDefinitionList);
            this.Update();
            if (this.FuelResources.Count <= 0)
            {
                this.Clear();
                this.Update();
                throw new ApplicationException("Must specify at least one fuel resource");
            }
            this.CheckResourcesForConflictingTypesAtHabitats();
        }
    }
}
