...
namespace DistantWorlds.Types
{
    ...
    public class GovernmentBiasList
    {
        ...
        public static void LoadFromFile(SQLiteDataReader reader, ref GovernmentAttributesList governments)
        {
            int id = 0;
            try
            {
                while (reader.Read())
                {
                    List<int> biasValues = new List<int>();
                    id = reader.GetInt32(reader.GetOrdinal("ID"));
                    if (id < 0)
                        throw new ApplicationException($"Wrong GovernmentId {id}");
                    string governmentName = reader.GetString(reader.GetOrdinal("Name"));
                    if (string.IsNullOrWhiteSpace(governmentName))
                        throw new ApplicationException($"Could not read Government Name at ID {id}");
                    string[] arr = reader.GetString(reader.GetOrdinal("Biases")).Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                    foreach (var item in arr)
                    {
                        if (int.TryParse(item, out int bias))
                        {
                            int num5 = Math.Max(-30, Math.Min(bias, 30));
                            biasValues.Add(num5);
                            if (biasValues.Count > governments.Count)
                                throw new ApplicationException($"More bias values than governments at ID {id}");
                        }
                        else
                        {
                            throw new ApplicationException($"Could not read Bias Value {item} at ID {id}");
                        }
                    }


                    if (biasValues.Count != governments.Count)
                        throw new ApplicationException($"Wrong number of bias values at ID {id} in Goverment Biases. Number of bias values should match number of governments, i.e. columns should match rows.");
                    if (id >= 0 && id < governments.Count)
                    {
                        List<KeyValuePair<int, int>> biases = new List<KeyValuePair<int, int>>();
                        for (int index = 0; index < biasValues.Count; ++index)
                            biases.Add(new KeyValuePair<int, int>(index, biasValues[index]));
                        governments[id].Biases.SetInternal(biases);
                    }
                }
            }
            catch (ApplicationException ex)
            ...
            {
                throw new ApplicationException($"Error at ID {id} in overment Biases");
            }
        }
