...
namespace DistantWorlds
{
	...
	public class EffectsPlayer
	{
		...
		private void method_0(string string_2, string string_3)
		{
			lock (this.@lock)
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					MemoryStream memoryStream = this.method_1(text3);
		...
		private unsafe SoundEffect LoadSoundEffect(Stream stream_0) {
		...
		public void PlayEffect(string filename, double balance, double volume, int frequency)
		{
			try
			{
				...
				SoundEffect obj = this.sfxBank[filename];
				...
				{
					secondaryBuffer = this.CreateSoundEffectInstance(text);
				}
				IL_FC:
				if (secondaryBuffer != null && !secondaryBuffer.IsDisposed)
				{
					...
					secondaryBuffer.Pan = num;
					secondaryBuffer.Volume = num2;
					if (frequency > 0)
					{
