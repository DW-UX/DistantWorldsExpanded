...
{
    internal class GovBiasConverter
    {
        private const string _tableName = "GovernmentBiases";
        ...
        public GovBiasConverter(ConvertType convertType)
        {
        ...
        public bool Parse(string filePath, string outputFolder)
        {
            ...
            {
                Console.WriteLine($"GovBias conversion error: {ex.Message}");
                res = false;
            }
            return res;
        }
        private void WriteXml(string filePath, string outputFolder, List<string[]> values)
        {
            ...
            for (int i = 0; i < values.Count; i++)
            {
                ...
                {
                    race.Value = $"UPDATE {_tableName} SET {_IdCol} = {values[i][0]}, {_NameCol} = '{values[i][1]}', {_BiasesCol} = '{biases}' WHERE {_NameCol} = '{values[i][1]}'";
                }
                else
                {
