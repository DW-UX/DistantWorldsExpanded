...
{
    internal class RaceFamiliesConverter
    {
        private const string _tableName = "RaceFamily";
        private const string _IdCol = "ID";
        private const string _RaceFamilyNameCol = "RaceFamilyName";
        private const string _SpecailFucCode = "SpecialFunctionCode";
        private readonly ConvertType convertType;

        public RaceFamiliesConverter(ConvertType convertType)
        {
        ...
        private void WriteXml(string filePath, string outputFolder, List<string[]> values)
        {
            ...
            {
                var race = new XElement("RaceFamilie");
                if (convertType == ConvertType.Update)
                {
                    race.Value = $"UPDATE {_tableName} SET {_RaceFamilyNameCol} = '{values[i][1]}', {_SpecailFucCode} = {values[i][2]} WHERE {_RaceFamilyNameCol} = '{values[i][1]}'";

                }
                else
                {
                    race.Value = $"INSERT INTO {_tableName} ({_IdCol}, {_RaceFamilyNameCol}, {_SpecailFucCode}) VALUES ({values[i][0]}, '{values[i][1]}', {values[i][2]})";
                }
                root.Add(race);
            }
            doc.Save(fileStream);
