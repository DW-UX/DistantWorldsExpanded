...
namespace TxtFileParser
{
    ...
    {
        private static readonly ImmutableList<string> _files = ImmutableList.Create<string>("characterNames.txt", "raceFamilies.txt", "raceFamilyBiases.txt", "plagues.txt", "races", "governments.txt", "GovernmentBiases.txt", "Facilities.txt", "Fighters.txt", "Resources.txt", "Research.txt", "RaceBiases.txt", "Components.txt", "Policy");
        ...
        static void Main(string[] args)
        {
            ...
            var policyParser = new PolicyConverter(convertType);
            int i = 0;

            if (!charParser.Parse(_files[i++], _outputFolder) || !raceFamParser.Parse(_files[i++], _outputFolder) || !raceFamBiasParser.Parse(_files[i++], _outputFolder) || !plagueParser.Parse(_files[i++], _outputFolder) || !raceParser.Parse(_files[i++], _outputFolder) || !govParser.Parse(_files[i++], _outputFolder) || !govBaiasParser.Parse(_files[i++], _outputFolder) || !facilitiesParser.Parse(_files[i++], _outputFolder) || !fightersParser.Parse(_files[i++], _outputFolder) || !fresourceParser.Parse(_files[i++], _outputFolder) || !researchParser.Parse(_files[i++], _outputFolder) || !raceBiasParser.Parse(_files[i++], _outputFolder) || !componentParser.Parse(_files[i++], _outputFolder) || !policyParser.Parse(_files[i++], _outputFolder))
            {
